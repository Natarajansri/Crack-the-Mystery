                ...
                <tbody className="divide-y divide-slate-800/60 text-slate-200">
                  {filteredRooms.length === 0 ? (
                    ...
                    filteredRooms.map((item) => (
                      <tr key={item.room.id} className="hover:bg-navy-850/50 transition-colors">
                        <td className="py-3 px-4 font-mono font-bold text-cyber-cyan">
                          {item.room.code}
                        </td>
                        <td className="py-3 px-4">
                          <div className="font-bold text-white">{item.room.teamName}</div>
      ...
      {selectedRoomState && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-navy-900 border border-cyber-cyan/40 rounded-2xl p-6 shadow-2xl my-8">
            ...
            <h3 className="text-base font-bold font-display uppercase text-white mb-1">
              Room Audit: {selectedRoomState.room.teamName} ({selectedRoomState.room.code})
            </h3>
            <p className="text-xs text-slate-400 mb-4">
              Status: {selectedRoomState.room.status} • Current Puzzle: {selectedRoomState.room.currentPuzzleNumber}
            </p>
